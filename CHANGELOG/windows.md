...
**构建**：GitHub Actions `release-windows.yml` · `windows-latest` · NSIS 安装包 · `createUpdaterArtifacts: true`
**制品**：`Otto_0.2.1_x64-setup.exe` + `.exe.sig`
**升级基址**：`http://121.43.110.135:8000/updates/`（Windows 直连 ECS）
...
- **应用内自动更新**：`tauri-plugin-updater` + `SystemUpdatePanel` / `UpdateDialog` / `composables/useUpdater.ts`
  - 更新源：`http://121.43.110.135:8000/updates/latest.json` → `windows-x86_64`
